...
void DirectXCommon::Initialize(WinApp* winApp)
{
	...
	//DXGIデバイス初期化
	if (!InitializeDXGIDevice()) { assert(0); }

	//コマンド関連初期化
	if (!InitializeCommand()) { assert(0); }

	//スワップチェーンの生成
	if (!CreateSwapChain()) { assert(0); }

	//レンダーターゲット生成
	if (!CreateFinalRenderTargets()) { assert(0); }
...
void DirectXCommon::PreDraw()
{
	...
	//ビューポートの設定
	commandList->RSSetViewports(1, &CD3DX12_VIEWPORT(0.0f, 0.0f, WinApp::winWidth, WinApp::winHeight));
	//シザリング矩形の設定
...
void DirectXCommon::ClearRenderTarget()
{
	...
	//全画面クリア
	float clearColor[] = { 0.61f, 0.69f, 0.92f,0.0f };
...
bool DirectXCommon::CreateSwapChain()
{
	...
	swapchainDesc.Width = WinApp::winWidth;
	swapchainDesc.Height = WinApp::winHeight;
